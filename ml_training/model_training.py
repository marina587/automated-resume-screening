...
class ResumeClassifier:
    ...
    def __init__(
        ...
        self.vectorizer = (
            None  # Lazy-initialized in _build_feature_matrix when backend='tfidf'
        )
    ...
    def _build_feature_matrix(
        ...
        # Lazy-init TfidfVectorizer only when using tfidf backend
    ...
    def train(
        ...
        When tune_hyperparams=True and model_type='logistic', performs a
        lightweight grid search over C values to find the best regularization
        strength. This is skipped for very small datasets (<200 samples).
        """
        ...
        print(
            f"Extracting features for {len(train_texts)} training resumes "
        ...
        if calibrate and len(y_train) >= 30:  # Need sufficient data for calibration
            ...
            print(
                "[OK] Model calibrated with CalibratedClassifierCV (isotonic method, cv=3)"
            )
        else:
            self.model.fit(X_train, y_train)
        ...
        if not holdout_df.empty and len(holdout_df) > 0:
            ...
            if self.feature_backend == "minilm":
                ...
                print(
                    f"Transforming {len(holdout_texts)} holdout resumes with TF-IDF..."
                )
            ...
            metrics["holdout_metrics"] = {
                ...
                "f1_macro": f1_score(holdout_labels, y_holdout_pred, average="macro"),
            }
